...
export async function initTaskpane(opts: {
  ...
  try {
    ...
    if (
      ...
      showToast("安全警告：代理 URL 不是 localhost — 它可以看到您的 token 和提示词。");
    }
  } catch {
    // ignore
  }
        ...
        if (!isAbort) {
          ...
          if (findTrailingContextOverflowError(agent.state)) {
            showErrorBanner(
              errorRoot,
              autoCompactEnabled
                ? "Context window exceeded — 安永 will try compacting older history and retrying once. If this error persists, run /compact, scope your request to a smaller range, or use a larger-context model."
  ...
  const replaceActiveRuntimeSession = async (sessionData: SessionData): Promise<void> => {
    ...
    if (busy) {
      showToast("当前标签页忙碌 — 请在新标签页打开或等待完成");
      return;
    }
  ...
  const reopenRecentlyClosedItem = async (item: RecentlyClosedItem): Promise<ReopenRecentlyClosedResult> => {
    try {
      ...
      await openSessionInNewTab(sessionData);
      showToast(`已重新打开：${formatSessionTitle(item.title)}`);
      return "reopened";
    } catch {
      showToast("无法重新打开会话");
      return "failed";
  ...
  const renameRuntimeTab = async (runtimeId: string): Promise<void> => {
    ...
    const nextTitleRaw = window.prompt("重命名标签页", defaultTitle);
  ...
  const closeOtherRuntimes = async (runtimeId: string): Promise<void> => {
    ...
    showToast(`已关闭 ${closedCount} 个其他标签页`);
  };
  ...
  const moveRuntimeTab = (runtimeId: string, direction: -1 | 1): void => {
    ...
    const directionLabel = direction < 0 ? "左移" : "右移";
    showToast(`已${directionLabel}标签页`, 1200);
  };
  ...
  const applyModelSelection = async (runtimeId: string, nextModel: RuntimeModel): Promise<void> => {
    ...
    showToast(`已在新标签页打开 ${modelForkTitle}`);
  };
  ...
  sidebar.onFilesDrop = (files: File[]) => {
    ...
    void workspace.importFiles(files, {
      ...
      .then((count) => {
        ...
        const importedLabel = `${count} 个文件`;
        showToast(`已导入 ${importedLabel} 到文件。`);
      })
      .catch((error: unknown) => {
  ...
  const openThinkingPopoverFrom = (target: Element): void => {
    ...
    const description = trigger.getAttribute("data-tooltip") ?? "选择安永 回答前的推理深度。";
