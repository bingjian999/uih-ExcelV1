...
function renderStatusBar(
  ...
  const modeTooltip = modeIsAuto
    ? "自动模式：安永 立即应用工作簿更改。点击切换到确认模式。"
    : "确认模式：安永 每次更改前询问。点击切换到自动模式。";
  ...
  const thinkingTooltip = escapeAttr(
    "安永 答题前的思考深度 — 越高越慢但更深入。点击选择，或 ⇧Tab 循环切换。",
  );
  ...
  const nextMarkup = `
    <div class="pi-status-main">
      ...
      <button type="button" class="pi-status-thinking pi-status-clickable" data-tooltip="${thinkingTooltip}" aria-label="思考级别 ${thinkingLevel}">${brainSvg} ${thinkingLevel}<span class="pi-status-affordance" aria-hidden="true">${affordanceChevronSvg}</span></button>
      <button type="button" class="pi-status-ctx pi-status-ctx--trigger pi-status-clickable has-tooltip" ${STATUS_CONTEXT_DESC_ATTR}="${ctxPopoverDesc}" ${STATUS_CONTEXT_TOKENS_ATTR}="${ctxPopoverTokens}" ${STATUS_CONTEXT_WARNING_ATTR}="${ctxPopoverWarnText}" ${STATUS_CONTEXT_WARNING_SEVERITY_ATTR}="${ctxWarningSeverity}" aria-label="上下文用量 ${pct}% / ${ctxLabel}"><span class="pi-status-ctx__pct ${ctxColor}">${pct}%</span><span class="pi-status-ctx__sep">/</span><span class="pi-status-ctx__limit">${ctxLabel}</span>${usageDebug}<span class="pi-status-affordance" aria-hidden="true">${affordanceChevronSvg}</span><span class="pi-tooltip"><span class="pi-tooltip__desc">${escapeHtml(ctxDescription)}</span><span class="pi-tooltip__tokens">${escapeHtml(ctxTokenDetail)}</span>${ctxWarning}</span></button>
      ${lockBadge}
    </div>
