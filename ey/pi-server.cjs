#!/usr/bin/env node
/**
 * 安永AI_Base — 本地启动器服务器 (CommonJS, SEA 兼容)
 *
...
// ---------------------------------------------------------------------------
// File logger — writes to %LOCALAPPDATA%\EY_AI_Base\server.log
...
const SEA_CACHE_DIR = SEA_INFO
  ? path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "EY_AI_Base")
  : null;
...
$cert = New-SelfSignedCertificate -DnsName 'localhost' -CertStoreLocation 'Cert:\\CurrentUser\\My' -FriendlyName '安永AI_Base (local)' -NotAfter (Get-Date).AddDays(825) -TextExtension @('2.5.29.17={text}DNS=localhost&IPAddress=127.0.0.1','2.5.29.37={text}1.3.6.1.5.5.7.3.1')
...
function tryNodeCryptoCert() {
  ...
  try {
    ...
    const ps = [
      "$dir = '" + CERT_DIR.replace(/\\/g, "\\") + "'",
      "$cert = New-SelfSignedCertificate -DnsName 'localhost','127.0.0.1','::1' -CertStoreLocation 'Cert:\\CurrentUser\\My' -FriendlyName 'EY_AI_Base (local)' -NotAfter (Get-Date).AddDays(825) -KeyExportPolicy Exportable -TextExtension @('2.5.29.17={text}DNS=localhost&IPAddress=127.0.0.1','2.5.29.37={text}1.3.6.1.5.5.7.3.1')",
...
<OfficeApp xmlns="http://schemas.microsoft.com/office/appforoffice/1.1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:bt="http://schemas.microsoft.com/office/officeappbasictypes/1.0" xsi:type="TaskPaneApp">
  ...
  <ProviderName>安永AI_Base</ProviderName>
  <DefaultLocale>zh-CN</DefaultLocale>
  <DisplayName DefaultValue="安永AI_Base" />
  <Description DefaultValue="安永AI_Base - 多模型 AI 侧边栏 Excel 加载项" />
...
function generateSideloadXlsx(addinId, addinVersion) {
  ...
  const ts = Date.now();
  const sideloadPath = path.join(tmpDir, `EY_AI_Base-sideload-${ts}.xlsx`);
  try {
...
function printInstructions(manifestFile, certInfo, localModels) {
  ...
  try {
    const dest = path.join(desktop, "EY_AI_Base-manifest.xml");
  ...
  console.log("     - CORS 代理 (端口 3003, 替代 npx ey-excel-proxy)");
  console.log("     - Excel 自动启动并加载安永 侧边栏");
  console.log("");
  ...
  if (names.length) {
    ...
    console.log("");
    console.log("  在 安永 -> /settings -> 自定义 OpenAI 兼容网关:");
  ...
  console.log("");
  console.log("  使用安永AI_Base 期间请保持此窗口打开。");
  console.log("  按 Ctrl+C 停止。\n");
}

main().catch((e) => {
  console.error("[安永] 致命错误:", e);
  process.exit(1);
});
